...
async function clickPreJoinControl(page: Page, text: string): Promise<boolean> {
  const labelPattern = new RegExp(text.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');

  const tryPlaywrightClick = async (scope: Page | Frame): Promise<boolean> => {
    try {
      const button = scope.getByRole('button', { name: labelPattern });
      if ((await button.count()) > 0) {
        await button.first().click({ force: true, timeout: 5000 });
        return true;
      }
      const link = scope.getByRole('link', { name: labelPattern });
  ...
  if (await tryPlaywrightClick(page)) return true;
  for (const frame of page.frames()) {
    if (await tryPlaywrightClick(frame)) return true;
  }
