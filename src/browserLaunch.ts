...
import { getBrowserProfileDir } from './appPaths';
import { getChromiumPolicyFile, getDismissDialogExePath, getPlaywrightChromiumExecutable } from './resources';
...
function ensureChromiumPolicyFile(): string {
  const policyFile = getChromiumPolicyFile();
...
export async function dismissNativeProtocolDialogBestEffort(page?: Page): Promise<boolean> {
  ...
  const dismissExe = getDismissDialogExePath();
  if (!fs.existsSync(dismissExe)) {
  ...
  for (const args of attempts) {
    try {
      await execFileAsync(dismissExe, args, { windowsHide: true, timeout: 20_000 });
...
export async function launchTeamsBrowser(): Promise<LaunchedBrowser> {
  ...
  const executablePath = getPlaywrightChromiumExecutable();
  if (executablePath) {
    console.log(`[browserLaunch] Using Chromium: ${executablePath}`);
  }
