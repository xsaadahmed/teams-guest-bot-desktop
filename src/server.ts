...
import { getRecordingsDir } from './appPaths';
...
/** Bind loopback only — matches Electron and avoids IPv4/IPv6 port-check mismatches on Windows. */
...
async function listenOnFirstFreePort(
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    try {
      const server = await new Promise<import('http').Server>((resolve, reject) => {
        const s = app.listen(port, LISTEN_HOST, () => resolve(s));
        s.on('error', reject);
      });
      if (i > 0) {
        console.warn(`[server] Port ${candidates[0]} is in use — using ${port} instead.`);
      }
      return { port, server };
    } catch (err) {
  ...
  throw new Error(`All candidate ports are in use: ${candidates.join(', ')}.${detail}`);
}
...
export interface StartServerOptions {
  /** When false, skip launching an external browser window (Electron provides its own). */
  openWebUi?: boolean;
}
...
export async function startServer(options: StartServerOptions = {}): Promise<{ port: number }> {
  ...
  const { port } = await listenOnFirstFreePort(getPortCandidates());
